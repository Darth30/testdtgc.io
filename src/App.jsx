...
export default function App() {
  ...
  const [livePrices, setLivePrices] = useState({
    urmom: BURN_STATS.urmomPrice || 0.0004,
    dtgc: BURN_STATS.dtgcPrice || 0.0006,
    marketCap: 0,
    dtgcMarketCap: 0,
    lastUpdated: null,
    loading: true,
  ...
  const handleStake = async () => {
    ...
    try {
      ...
      if (isLP) {
        ...
        stakeTx = await stakingContract.stake(amountWei, lpType, txOptions);
      } else {
  ...
  const handleUnstake = async (positionId) => {
    ...
    try {
      ...
      setLoading(false);
      showToast('✅ Successfully withdrawn! Check Gold Records.', 'success');
            ...
            <section className="section">
              ...
              {selectedTier !== null && account && (
                <div className="staking-panel">
                  ...
                  {/* Gas Speed Selector (Mainnet only) */}
                  {!TESTNET_MODE && (
                    <div style={{
                      display: 'flex',
                      gap: '8px',
